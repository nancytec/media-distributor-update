...
class ChurchMediaStatController extends BaseController
{

    public function getUserMediaLikes()
    {
        $referral_link = url()->previous();
        $results = ChurchMediaLinkLike::where('media_link', $referral_link)->get();
        return $this->sendResponse(count($results));
    }

    public function getUserMediaViews(Request $request)
    {
        $results = ChurchMediaLinkView::where('media_link', $request->path_name)->get();
        return $this->sendResponse(count($results));
    }

    public function getUserMediaDownloads(Request $request)
    {
        $results = CHurchMediaLinkDownload::where('media_link', $request->path_name)->first();
        if ($results){
    ...
    {
        $results = ChurchMediaLinkShare::where('media_link', $request->path_name)->first();
        if ($results){
            return $this->sendResponse($results->count);
        }
        return 0;
...
//        ]);
    ...
    public function updateDownloads(Request $request){
        $link = ChurchMediaLinkDownload::where('media_link', $request->path_name)->first();
        if ($link){
            $link->count += 1;
            $link->save();
    ...
    {
        $link = ChurchMediaLinkShare::where('media_link', $request->path_name)->first();
        if ($link){
            $link->count += 1;
            $link->save();
            return $this->sendResponse($request->all(), 'It got here');
    ...
    {
        //1.) Update the views
        $member = User::where('slug', $request->slug)->first();
        if (ChurchMediaLinkView::where([
            ['media_link', '===', $request->path_name],
            ['ip_address', '===', \request()->ip()]
        ])->first()){
            return $this->sendResponse($member, 'Existing');
        }else{
            ChurchMediaLinkView::create([
                'media_link' => $request->path_name,
                'church_id' => $member->id,
                'ip_address' => \request()->ip()
            ]);
